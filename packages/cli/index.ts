...
function usesPostgres(selection: DbSelection): boolean {
  return (
    selection === "postgres" ||
    selection === "postgres-neon" ||
    selection === "postgres-supabase"
  );
}
...
function buildContainerFile(options: InstallerOptions): string {
  ...
  if (options.db === "postgres" || options.db === "postgres-neon" || options.db === "postgres-supabase") {
    todoRepoFactory = [
      ...
      "  databaseUrl,",
      "  localDbPath: process.env.TODO_PGLITE_PATH ?? \"./data/pglite\",",
...
function buildRootPackageJson(options: InstallerOptions): Record<string, unknown> {
  return {
    ...
    scripts: {
      ...
      "dev:api": "tsx watch --clear-screen=false ./scripts/dev-api.ts",
...
function buildEnvExample(options: InstallerOptions): string {
  const lines = [
    ...
    "# Leave DATABASE_URL empty to use local embedded Postgres-compatible storage (PGlite)",
    "DATABASE_URL=",
    "TODO_PGLITE_PATH=./data/pglite",
...
function buildPostgresDockerCompose(): string {
  return `services:
  postgres:
    image: postgres:16-alpine
    container_name: teehex-local-postgres
    environment:
      POSTGRES_DB: teehex
    ...
    ports:
      - "54329:5432"
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U postgres -d teehex"]
      interval: 3s
      timeout: 3s
      retries: 20
    volumes:
      - teehex-postgres-data:/var/lib/postgresql/data

volumes:
  teehex-postgres-data:
`;
}
...
- One command dev loop:
  - pnpm dev
  - Runs a local Node API server on port 3000 and Vite frontend dev server.
- For Postgres selections, local dev works without env vars via embedded Postgres-compatible storage (PGlite).
- If \`DATABASE_URL\` is provided, it uses your real Postgres provider instead.
- Frontend API calls are proxied to http://localhost:3000/api.
- Local API runner auto-loads \`.env.local\` and \`.env\`.
- Optional Vercel parity mode (requires vercel login):
  - pnpm dev:vercel
...
- Fill values in \`.env\` (pre-generated from \`.env.example\`) for your provider.
- TODO_DB_PROVIDER=${toDbProviderLabel(options.db)}
- TODO_SQLITE_PATH=./data/app.sqlite
- DATABASE_URL=
- TODO_PGLITE_PATH=./data/pglite
...
## Performance and GC (short)

- Do not try to control GC by retaining references; that often increases memory pressure and pause times.
- Prefer: fewer allocations, stable object shapes, buffer reuse, streaming, and worker offload for CPU-heavy tasks.
- Use --expose-gc only at batch boundaries and only after measuring pause impact.
- Use --max-old-space-size when workloads are truly large and profiled; do not use it to mask leaks.
`;
}
...
async function scaffoldProject(options: InstallerOptions): Promise<void> {
  ...
  await copyDirectory(join(backendRoot, "scripts"), join(projectDir, "scripts"));
  ...
  const envTemplate = buildEnvExample(options);
  await writeFile(join(projectDir, ".env.example"), envTemplate, "utf8");
  await writeFile(join(projectDir, ".env"), envTemplate, "utf8");
  ...
  if (usesPostgres(options.db)) {
    await writeFile(join(projectDir, "docker-compose.yml"), buildPostgresDockerCompose(), "utf8");
  }
